...
from iarc_sim_engine.srv import SpawnRobot, SpawnRobotRequest, SpawnRobotResponse, KillRobot, KillRobotRequest, KillRobotResponse

from iarc_sim_2d.robots import Drone
from iarc_sim_2d import config as cfg
...
class SimManager(object):
    def __init__(self):
        ...
        self.start_sub_ = rospy.Subscriber('/start_round', StartRound, self.start_cb)

    @staticmethod
    def spawn_robot(client, name, img, radius, pose, robot_class):
        """
        Generic function for spawning any robot.
        """
        try:
            client(name=name,img=img,radius=radius,x=pose.x,y=pose.y,t=pose.theta)
        except rospy.ServiceException as e:
            rospy.logerr_throttle(1.0, 'Spawning %s Failed : %s' % (name, e) ) #replace with publisher
            return None
    ...
    def kill_robot(name):
        ...
        try:
            client = rospy.ServiceProxy('/kill', KillRobot)
            return client(name)
        except rospy.ServiceException as e:
    ...
    def start_cb(self, msg):
        self.start_time_ = msg.time
        if msg.start and self.start_time_.to_sec() == 0:
    ...
    def spawn_drone(self):
        ...
        img = os.path.join(
                rospack.get_path('iarc_sim_engine'),
                'data', 'drone.jpg')
    ...
    def reset(self):
        ...
        raise NotImplementedError("Resetting is not currently supported")

    def update(self, delta, elapsed):
        """ update with single step """
        self.drone_.update(delta, elapsed)
        self.z_pub_.publish(self.drone_.pos3d[2])
...
#
#            Vis_Roomba_main.last_turn = self.start_time_
